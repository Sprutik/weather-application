// ====Work with API ====
{
  ...
  if (night) {
    document.querySelector('.section-header').style.background = 'url(img/night.png) center/cover'
  ...
  cities.forEach((name) => {
    ...
    card.style.background = `url(img/cities/${name.toLocaleLowerCase().replace(/\s+/g, '')}.jpg)`
  ...
  function drawLastVisitedList(cityName) {
    ...
    if (lastVisited.length > 0) {
      ...
      const lastVisitedList = document.querySelector('.section-header-last-visited')
  ...
  searchBtn.addEventListener('click', (e) => {
    e.preventDefault()
    const inputValue = document.querySelector('.section-header-search-input').value
    if (inputValue) {
      fetch(
  ...
  function drawDropdownList(list) {
    ...
    if (list.length == 0) {
      ...
      list.forEach((element) => {
        ...
        const cityName = `${element.name}, ${new Intl.DisplayNames(['en'], { type: 'region' }).of(element.sys.country)}`
  ...
  function changeWeather(element) {
    ...
    document.querySelector('.weather-name').textContent = element.weather[0].main
    document.querySelector('.weather-conditions').textContent = element.weather[0].description
    document.querySelector('.location').textContent = `${element.name}, ${new Intl.DisplayNames(['en'], {
      type: 'region',
    }).of(element.sys.country)}`
    document.querySelector('.temperature').textContent = Math.floor(element.main.temp) + '°C'
    document.querySelector('.max-temperature').textContent = Math.floor(element.main.temp_max) + '°C'
    document.querySelector('.min-temperature').textContent = Math.floor(element.main.temp_min) + '°C'
  }
